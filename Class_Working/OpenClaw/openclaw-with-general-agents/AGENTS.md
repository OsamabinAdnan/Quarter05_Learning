...
### NemoClaw: the external cage (OpenShell)

`sandbox.mode` above is OpenClaw's own in-process isolation. **NemoClaw** is the heavier, external option: NVIDIA's tool that wraps the whole Employee inside an **OpenShell** cage (Linux-kernel isolation via Landlock, seccomp, netns) confined to one workspace folder and an allow-listed network. Reach for it before the Employee reads messages from anyone but the owner. The threat is prompt injection; the cage is the structural defense. Pairing controls _who_ can write to the Employee; the cage controls _what it can do with what it reads_.

**NemoClaw ships an agent-driven path built for exactly this job. Use it, and treat its live docs as the source of truth.** It is fast-moving beta, so do not run its commands from memory. Load its docs first, then let it self-direct, one question at a time:

- Add its docs MCP server, then ask it to walk you through setup: `claude mcp add --transport http nemoclaw-docs https://docs.nvidia.com/nemoclaw/_mcp/server` (OpenCode: add the same URL as an MCP server). Index: `https://docs.nvidia.com/nemoclaw/llms.txt`. OpenClaw quickstart: `https://docs.nvidia.com/nemoclaw/user-guide/openclaw/get-started/quickstart.md`.
- Its own skill is `nemoclaw-user-guide` (a sparse checkout of `github.com/NVIDIA/NemoClaw`), if you prefer a skill to the MCP server.

**Foundation first: the cage needs a Linux container engine, so confirm one is present before installing.**

- **macOS:** Colima or Docker Desktop running. `brew install colima docker && colima start`. NemoClaw uses the Docker-driver OpenShell path here.
- **Windows:** WSL2 plus Docker. See `.../openclaw/get-started/prerequisites/windows-preparation.md`.
- **Linux:** Docker Engine, native.

**Install (stable one-liner):** `curl -fsSL https://www.nvidia.com/nemoclaw.sh | bash`. This runs NVIDIA's last-known-good installer and launches the onboard wizard.

**Onboard is a human seam, like the channel-login TTY.** `nemoclaw onboard` is interactive: pick a provider (options 1 to 9), a model, and paste an API key. Do not drive it through the shell tool; hand it to the human and wait. Retry an interrupted run with `--resume`, reset with `--fresh`. There is no `--no-gpu` flag, and a GPU is not needed for hosted inference; pick a hosted provider such as OpenRouter in the wizard.

**Seams to name and wait on:** the Windows admin prompt plus reboot (WSL), the Linux or sandbox login, the onboard wizard, and the provider API key. Everything between (installing Docker, running the installer, reading logs, recovering) is agent work.

**Verify and recover:** open the dashboard with `nemoclaw <sandbox-name> dashboard-url --quiet`. If the link will not load, read the troubleshooting page (`.../openclaw/reference/troubleshooting.md`); a common fix is using the `127.0.0.1` address rather than the printed internal `172.x` one. For any command not covered here, read NemoClaw's CLI reference (`.../openclaw/reference/commands.md`); live docs win. Done when the Employee replies in the dashboard AND can see nothing outside its one workspace folder.
